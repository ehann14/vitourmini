...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        /* =========================================
           PANORAMA CONTAINER - TRUE FULLSCREEN
           ========================================= */
        #panorama-wrapper {
            position: fixed;
            top: 0;
        ...
        /* Override Pannellum default styles untuk fullscreen sempurna */
        .pnlm-container {
            width: 100vw !important;
            height: 100vh !important;
            height: 100dvh !important;
        ...
        /* Hide Pannellum controls yang tidak perlu */
        .pnlm-controls {
            opacity: 0.6;
        ...
        /* Khusus tombol Home agar lebih menonjol */
        .tool-btn.btn-home {
            background: rgba(255, 255, 255, 0.15);
            border-color: rgba(255, 255, 255, 0.4);
        }
        .tool-btn.btn-home:hover {
        ...
        .spinner-fs {
            width: 60px;
            height: 60px;
            border: 4px solid rgba(0, 201, 177, 0.2);
...
<body>
    ...
    <!-- PANORAMA LAYER - TRUE FULLSCREEN -->
    ...
    <div class="ui-overlay">
        ...
        <!-- Top Navbar -->
        <div class="navbar-fs">
            <!-- Logo juga berfungsi sebagai tombol kembali ke Home -->
            ...
            <div class="nav-actions">
                <button class="nav-icon-btn" onclick="toggleSceneSelector()" title="Pilih Lokasi">
                    <i class="fas fa-th-large"></i>
                </button>
                <button class="nav-icon-btn" onclick="openRoomInfoModal()" title="Informasi Ruangan">
        ...
        <div class="search-fs-container">
            <div class="search-fs-wrapper">
                ...
                <button class="search-fs-btn" id="searchHeroBtn" type="button">
        ...
        <!-- Bottom Toolbar (Ditambahkan Tombol Home) -->
        <div class="toolbar-fs">
            <button class="tool-btn btn-home" onclick="window.location.href='{{ route('home') }}'" title="Kembali ke Beranda">
                <i class="fas fa-home"></i>
                <span>Home</span>
            </button>
            <button class="tool-btn" onclick="openDenahModal()">
                <i class="fas fa-map"></i>
                <span>Denah</span>
            </button>
            <button class="tool-btn" onclick="toggleSceneSelector()">
                <i class="fas fa-list"></i>
                <span>Daftar</span>
            </button>
            <button class="tool-btn" onclick="resetSearchResult()">
    ...
    <div class="modal-overlay" id="sceneSelectorOverlay" onclick="closeSceneSelectorOnOverlay(event)">
        <div class="modal-content" onclick="event.stopPropagation()">
            ...
            <div class="modal-body">
                ...
                <div id="sceneButtons" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(150px, 1fr)); gap: 10px; max-height: 50vh; overflow-y: auto;">
        ...
        <div class="modal-content" style="max-width: 800px;" onclick="event.stopPropagation()">
            ...
            <div class="modal-body">
                ...
                <div style="margin-top: 20px;">
                    <h4 style="font-weight: 800; color: var(--primary-blue); margin-bottom: 15px;">Daftar Ruangan</h4>
                    <div style="position: relative; margin-bottom: 15px;">
                        <i class="fas fa-search" style="position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #999;"></i>
                        <input type="text" id="roomSearchInput" style="width: 100%; padding: 12px 15px 12px 45px; border: 2px solid #eee; border-radius: 12px; font-family: 'Poppins'; font-weight: 600; outline: none;" placeholder="Filter ruangan...">
                    </div>
                    <div id="roomListGrid" style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 10px; max-height: 200px; overflow-y: auto;"></div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-body">
                ...
                <div class="info-grid">
                    <div class="info-card">
                        ...
                        <div>
                            <div class="info-card-label">Kursi</div>
                            <div class="info-card-value" id="roomInfoKursi">-</div>
                        </div>
                    </div>
                    <div class="info-card">
                        <div class="info-card-icon"><i class="fas fa-table"></i></div>
                        <div>
                            <div class="info-card-label">Meja</div>
                            <div class="info-card-value" id="roomInfoMeja">-</div>
                        </div>
                    </div>
                    <div class="info-card" id="roomInfoPcItem">
                        <div class="info-card-icon"><i class="fas fa-desktop"></i></div>
                        <div>
                            <div class="info-card-label">Komputer</div>
                            <div class="info-card-value" id="roomInfoPc">-</div>
                        </div>
                    </div>
                    <div class="info-card">
                        <div class="info-card-icon"><i class="fas fa-ruler-combined"></i></div>
                        <div>
                            <div class="info-card-label">Ukuran</div>
                            <div class="info-card-value" id="roomInfoUkuran">-</div>
                        </div>
                    </div>
                </div>
    ...
    async function showScene(sceneId) {
        ...
        if (sceneData.isFlat) {
            ...
            sceneData.rawHotspots.forEach(h => {
                ...
                if (h.link) {
                    pin.onclick = () => selectScene(h.link, scenesConfig[h.link]?.title);
                }
                ...
                try {
                    viewer = pannellum.viewer('panorama', {
                        default: {
                            ...
                            compass: false,
                            hfov: window.innerWidth < 768 ? 110 : 100,
                            autoRotate: -2,
                            friction: 0.15
                        },
        ...
        denahData.forEach(room => {
            ...
            if (sceneKey) {
                roomInfoData[sceneKey] = {
                    name: room.name,
                    meja: room.jumlah_meja || 0,
                    kursi: room.jumlah_kursi || 0,
                    pc: room.jumlah_pc || 0,
                    ukuran: room.ukuran_ruangan || '-',
    ...
    function renderDenahPins() {
        ...
        denahData.forEach(room => {
            ...
            const pin = document.createElement('div');
            pin.className = 'denah-cluster-pin';
    ...
    function renderSearchDropdown(query) {
        ...
        results.slice(0, 6).forEach(room => {
            const item = document.createElement('div');
