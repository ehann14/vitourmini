                            ...
                            <div class="section-card">
                                ...
                                <div class="card-body p-0">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-hover align-middle mb-0">
                                            <thead class="table-light">
                                                <tr><th width="50">Avatar</th><th>Nama</th><th>Email</th><th>Pesan</th><th width="150">Tanggal</th><th width="80">Aksi</th></tr>
                                            </thead>
                                            <tbody>
                                                @foreach($pendingComments->take(5) as $comment)
                                                <tr>
                                                    <td><div class="comment-avatar">{{ strtoupper(substr($comment->name, 0, 1)) }}</div></td>
                                                    <td><div class="fw-bold">{{ $comment->name }}</div></td>
                                                    <td><small class="text-muted">{{ $comment->email }}</small></td>
                                                    <td><div class="comment-preview" title="{{ $comment->message }}">{{ $comment->message }}</div></td>
                                                    <td>
                                                        <small class="text-muted">{{ $comment->created_at->format('d M Y, H:i') }}</small><br>
                                                    ...
                                                    <td>
                                                        <form action="{{ route('admin.comments.toggle', $comment->id) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-sm btn-success py-0 px-2" title="Setujui"><i class="fas fa-check"></i></button>
                                                        </form>
                                                        <form action="{{ route('admin.comments.destroy', $comment->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Hapus komentar ini?')">
                                                            @csrf @method('DELETE')
                                                            <button type="submit" class="btn btn-sm btn-danger py-0 px-2" title="Hapus"><i class="fas fa-trash"></i></button>
                                                        </form>
                                                    </td>
                                                </tr>
                                                @endforeach
                                    ...
                                    @else
                                    <div class="empty-state py-4"><i class="fas fa-comments"></i><p class="mb-0">Tidak ada komentar yang menunggu persetujuan</p></div>
                                    @endif
                                </div>
                            </div>
                        </div>
